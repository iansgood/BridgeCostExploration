...
tr = struct('length',25,'width',3,'deflectionMax',0.75,'loadDist',12.5,'bridgeLength',60*fttoinch,'loadMax',500,'costLength',2); %in in, in, in, in, in, lb/in, and lb/in. 
...
% Solve for Thickness
[tYeild tDef] = thickness(tr,yeild,E); 
t = max(tYeild,tDef);
...
[price,massBridge,volBridge] = bridgeCost(tr,density,t,cost); % $, lb, in^3
price = round(price,2);% in USD
BridgeAlPrice = 5508; 
